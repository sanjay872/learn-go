...
To run a thread concurrently.

# Channel

To sent message in one goroutine and receive message from different goroutine.

we can also make a buffer in it, by adding number of message it a hold before it is consumed.

we create functions to decide the direction of channel.

# Select

select gives the ability to wait for both the channels to complete their job.

# Timeout

By using select, channel and time, we can wait for certain time, for the request to completed.

# Non Blocking Channels

We can have multiple channels monitored on the select and also have default response if no data was received in any channel.

# Closing Channels

We can close the channel after completing the task.

# Range over channel

Even when the channel is closed, we can get the remaining value from the channel using range.

# Timer And Ticker

We can create timers, they are channels and can be used to make the execution wait for some time.

Ticker is same as timer, but it will execute a certain interval of time.